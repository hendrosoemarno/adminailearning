            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-900/50 border-b border-slate-700">
                        ...
                        <th class="p-4 text-xs font-bold text-slate-400 uppercase tracking-wider">
                            <a href="{{ request()->fullUrlWithQuery(['sort' => 'firstname', 'direction' => ($sort == 'firstname' && $direction == 'asc') ? 'desc' : 'asc']) }}"
                                class="flex items-center gap-1 hover:text-white transition-colors">
                                Nama Siswa
                                @if($sort == 'firstname')
                                    @if($direction == 'asc')
                        ...
                        <th class="p-4 text-xs font-bold text-slate-400 uppercase tracking-wider">
                            <a href="{{ request()->fullUrlWithQuery(['sort' => 'paket_kode', 'direction' => ($sort == 'paket_kode' && $direction == 'asc') ? 'desc' : 'asc']) }}"
                                class="flex items-center gap-1 hover:text-white transition-colors">
                                Paket
                                @if($sort == 'paket_kode')
                                    @if($direction == 'asc')
                        ...
                        <th class="p-4 text-xs font-bold text-slate-400 uppercase tracking-wider">
                            <a href="{{ request()->fullUrlWithQuery(['sort' => 'tanggal_masuk', 'direction' => ($sort == 'tanggal_masuk' && $direction == 'asc') ? 'desc' : 'asc']) }}"
                                class="flex items-center gap-1 hover:text-white transition-colors">
                                Tanggal Masuk
                        ...
                        <th class="p-4 text-xs font-bold text-slate-400 uppercase tracking-wider">
                            <a href="{{ request()->fullUrlWithQuery(['sort' => 'biaya', 'direction' => ($sort == 'biaya' && $direction == 'asc') ? 'desc' : 'asc']) }}"
                                class="flex items-center gap-1 hover:text-white transition-colors">
                                Biaya
                                @if($sort == 'biaya')
                                    @if($direction == 'asc')
                        ...
                        <th class="p-4 text-xs font-bold text-slate-400 uppercase tracking-wider">
                            <a href="{{ request()->fullUrlWithQuery(['sort' => 'ai_learning', 'direction' => ($sort == 'ai_learning' && $direction == 'asc') ? 'desc' : 'asc']) }}"
                                class="flex items-center gap-1 hover:text-white transition-colors">
                                AI Learning
                                @if($sort == 'ai_learning')
                                    @if($direction == 'asc')
                        ...
                        <th class="p-4 text-xs font-bold text-slate-400 uppercase tracking-wider">
                            <a href="{{ request()->fullUrlWithQuery(['sort' => 'gaji_tentor', 'direction' => ($sort == 'gaji_tentor' && $direction == 'asc') ? 'desc' : 'asc']) }}"
                                class="flex items-center gap-1 hover:text-white transition-colors">
                                Gaji Tentor
                                @if($sort == 'gaji_tentor')
                                    @if($direction == 'asc')
                        ...
                        <th class="p-4 text-xs font-bold text-slate-400 uppercase tracking-wider">
                            <a href="{{ request()->fullUrlWithQuery(['sort' => 'total_meet', 'direction' => ($sort == 'total_meet' && $direction == 'asc') ? 'desc' : 'asc']) }}"
                                class="flex items-center gap-1 hover:text-white transition-colors">
                                Total Meet
                                @if($sort == 'total_meet')
                                    @if($direction == 'asc')
                ...
                <tbody class="divide-y divide-slate-700/50" id="sortable-students">
                    @forelse($siswas as $siswa)
                        <tr class="hover:bg-slate-700/20 transition-colors" data-siswa-id="{{ $siswa->id }}">
                            ...
                            <td class="p-4">
                                <input type="date" value="{{ $siswa->tanggal_masuk }}"
                            ...
                            <td class="p-4 text-sm text-emerald-400 font-medium whitespace-nowrap"
                                id="ai-learning-{{ $siswa->id }}">
                                Rp {{ number_format($siswa->ai_learning, 0, ',', '.') }}
                            </td>
                            <td class="p-4 text-sm text-amber-400 font-medium whitespace-nowrap"
                                id="gaji-tentor-{{ $siswa->id }}">
                                Rp {{ number_format($siswa->gaji_tentor, 0, ',', '.') }}
                            </td>
                            <td class="p-4">
                                <div class="flex items-center gap-2">
                                    <input type="number" min="1" max="99" value="{{ $siswa->total_meet }}"
                                    ...
                                    <span class="text-xs text-slate-500">/ <span
                                            id="default-meet-{{ $siswa->id }}">{{ $siswa->default_total_meet }}</span></span>
                                    @if($siswa->is_custom)
                                        <span class="text-xs text-amber-400" id="custom-indicator-{{ $siswa->id }}"
                                            title="Custom">🟡</span>
                                    @else
                                        <span class="text-xs text-green-400" id="custom-indicator-{{ $siswa->id }}"
                                            title="Default">🟢</span>
                                    @endif
                                </div>
                            </td>
