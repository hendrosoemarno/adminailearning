...
@section('content')
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold text-white tracking-tight">Ubah Data Admin</h1>
        <p class="text-slate-400">Silakan sesuaikan informasi untuk akun administrator berikut.</p>
    </div>

    <div class="max-w-2xl">
        <form action="{{ route('useradmins.update', $admin->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-2xl p-8 shadow-xl space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="nama" required value="{{ old('nama', $admin->nama) }}"
                ...
                <!-- Password -->
                <div>
                    <label class="block text-sm font-semibold text-slate-300 mb-2">Ganti Password (Kosongkan jika tidak
                        diubah)</label>
                    <input type="password" name="password" placeholder="Masukkan password baru jika ingin diubah..."
