...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            <h1 class="text-3xl font-bold text-white tracking-tight mb-2">Placement Test</h1>
            <p class="text-slate-400">Daftar siswa yang mengikuti kuis dengan nama mengandung "Placement" dan terpantau Enrolled.</p>
        </div>
    </div>
                ...
                <thead class="bg-slate-900/50 border-b border-slate-700/50 text-slate-400">
                    <tr>
                        <th class="p-4 font-semibold text-center">
                            <a href="{{ request()->fullUrlWithQuery(['sort' => 'timestart', 'direction' => ($sort === 'timestart' && $direction === 'asc') ? 'desc' : 'asc']) }}" class="flex items-center justify-center gap-1 hover:text-white transition-colors">
                                NO
                                @if($sort === 'timestart') <span class="text-xs">{{ $direction === 'asc' ? '↑' : '↓' }}</span> @endif
                            </a>
                        </th>
                        <th class="p-4 font-semibold">
                            <a href="{{ request()->fullUrlWithQuery(['sort' => 'quiz_date', 'direction' => ($sort === 'quiz_date' && $direction === 'asc') ? 'desc' : 'asc']) }}" class="flex items-center gap-1 hover:text-white transition-colors">
                                TANGGAL KUIS
                                @if($sort === 'quiz_date') <span class="text-xs">{{ $direction === 'asc' ? '↑' : '↓' }}</span> @endif
                            </a>
                        </th>
                        <th class="p-4 font-semibold">
                    ...
                    @forelse($attempts as $index => $attempt)
                        <tr class="hover:bg-slate-700/20 transition-colors">
                            ...
                            <td class="p-4 text-xs font-mono text-slate-400">
                                {{ $attempt->quiz_date ? date('Y-m-d H:i', $attempt->quiz_date) : '-' }}
                            </td>
                        ...
                        <tr>
                            <td colspan="7" class="p-12 text-center text-slate-500">
                                <div class="flex flex-col items-center justify-center gap-3">
                                    ...
                                    <span class="text-sm">Tidak ada data siswa yang memenuhi kriteria ujian Placement Test.</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
