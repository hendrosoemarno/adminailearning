...
@section('content')
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold text-white tracking-tight mb-2">Ubah Tarif</h1>
        <p class="text-slate-400">Silakan sesuaikan pembagian tarif di bawah ini.</p>
    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="mapel" required
                            ...
                            <option value="" {{ $tarif->mapel == '' ? 'selected' : '' }}>Umum / Semua</option>
                            <option value="mat" {{ $tarif->mapel == 'mat' ? 'selected' : '' }}>Matematika</option>
                            <option value="bing" {{ $tarif->mapel == 'bing' ? 'selected' : '' }}>Bahasa Inggris</option>
                            <option value="coding" {{ $tarif->mapel == 'coding' ? 'selected' : '' }}>Coding</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-300 mb-2">Kode</label>
                        <input type="text" name="kode" required value="{{ $tarif->kode }}"
                ...
                <div class="p-6 bg-slate-900/50 rounded-xl border border-slate-700/50 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="number" name="aplikasi" required value="{{ $tarif->aplikasi }}"
                        ...
                        <div>
                            ...
                            <input type="number" name="manajemen" required value="{{ $tarif->manajemen }}"
                        ...
                        <div>
                            ...
                            <input type="number" name="tentor" required value="{{ $tarif->tentor }}"
